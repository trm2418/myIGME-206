...
namespace Final_Project
{
    ...
    public class Skill
    {
        ...
        public string targetParty = "enemies";
        ...
        // percentage of health healed by the skill
        public float healPercent = 0;

        // who the skill heals
        public string healTargeting = "self";

        // some skills only heal when a certain condition is met
        public string healCondition = "";
        ...
        public HitAgainStyle style = null;
                    ...
                    else if (healTargeting == "all")
                    {
                        ...
                        foreach (Battler battler in target.party.members)
                        {
                            ...
                            if (battler != null)
                            {
                                ...
                                double amount = battler.maxHp * healPercent;
        ...
        public void HitAgain(Battler user, Battler target, HitAgainStyle style)
        {
            ...
            if (style.condition == "killed target")
            {
                while (target.dead && !target.party.IsDefeated() && hits <= style.maxHits)
                {
                    ...
                    baseDamage *= (int)Math.Round(style.damageMult);
            ...
            {
                if (target.partySpot % 2 == 0 && target.GetBehind() != null)
                {
            ...
            {
                int spot = target.partySpot;
                if (spot > 1 && spot < 6)
                {
                    if (target.party.members[spot - 2] != null)
                    {
                        DealDamage(user, target.party.members[spot - 2]);
                    }
                    if (target.party.members[spot + 2] != null)
                    {
                        DealDamage(user, target.party.members[spot + 2]);
        ...
        public void TriggerEffects(Battler user, Battler target)
        {
            ...
            if (effects != null)
            {
                for (int i = 0; i < effects.Length; i++)
                {
                    ...
                    if ((target.hp > 0 || new string[] { "Disarm", "HitAgain", "Heal" }.Contains(effects[i].name)) && healCondition != "kill")
                    {
                    ...
                    else if ((target.hp <= 0 && healCondition == "kill") || (target.sentinelSave && !target.party.hasSentinel))
                    {
                        if (target.sentinelSave)
                        {
                            Effect(user, target.party.GetMemberByName("Sentinel"), i);
        ...
        public void Use(Battler user, Battler target)
        {
            ...
            if (!ranged && targeting != "self" && user.statusConditions.ContainsKey("Bleed") && !user.multiHit)
            {
            ...
            if (user.hp > 0)
            {
                ...
                // hit all skills
                // Sentinel should save the first ally to get hit if they are going to die but I haven't implemented this yet
        ...
        public Skill(string name, int manaCost, string targeting, int baseDamage, bool ranged, params SkillEffect[] effects)
        {
            this.name = name;
