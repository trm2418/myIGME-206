...
{
    // Class: PeopleApp
    // Purpose: PE12 Question 7
    // Author: Tyler Machanic and David Schuh
    // Restrictions: None
    class PeopleApp
    {
        // Method: Main
        // Purpose: Lets user add, edit and delete people, display all of them at once
        //          have them live a day, and quit the program
        // Output: Input prompts, list of people, messages from LiveADay
        // Restrictions: course codes must be all caps and have a hyphen between IGME and the number
        static void Main(string[] args)
        {
            // create sorted list of 100 courses with random schedules
                        ...
                        foreach (KeyValuePair<string, Person> thisEntry in people.sortedList)
                        {
                            ...
                            if (thisPerson.GetType() == typeof(Student))
                            {
                                ...
                                foreach (string courseCode in student.courseCodes)
                                {
                                    ...
                                    // print code and description with formatting
                                    Console.Write("\t" + courseCode + " - " + thisCourse.description + " ");

                                    // loop through each day in the course's  schedule
                                    foreach (DayOfWeek day in thisCourse.schedule.daysOfWeek)
                                    {
                                        // print the day
                                        Console.Write(day + " ");
                                    }
                                    // print the start and end time and end the line
                                    Console.WriteLine($"{thisCourse.schedule.startTime:hh:mmtt} - {thisCourse.schedule.endTime:hh:mmtt}");
                                }
                            }
